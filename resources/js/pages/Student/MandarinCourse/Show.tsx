            ...
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
                    ...
                    <div className="space-y-6 lg:col-span-8">
                        {activeLesson && (
                            <>
                                {/* KONDISIONAL MULTIMEDIA: Merender Elemen Berdasarkan Jenis Content Type */}
                                
                                {/* 1. JIKA TIPE VIDEO */}
                                {activeLesson.content_type === 'video' && (
                                    <div className="overflow-hidden rounded-2xl border border-gray-100 bg-black shadow-xs aspect-video">
                                        {getYouTubeEmbedUrl(activeLesson.video_url) ? (
                                            ...
                                            <video className="w-full h-full" controls src={`/storage/${activeLesson.video_path}`} />
                                        ) : (
                                            <div className="flex h-full items-center justify-center text-sm text-gray-400">Media Video Tidak Ditemukan</div>
                                        )}
                                    </div>
                                )}

                                {/* 2. JIKA TIPE AUDIO */}
                                {activeLesson.content_type === 'audio' && activeLesson.lesson_audio_path && (
                                    <div className="flex flex-col gap-3 rounded-2xl border border-gray-100 bg-linear-to-r from-blue-50 to-indigo-50 p-5 shadow-xs">
                                        <div className="flex items-center gap-2.5 text-xs font-bold text-blue-700 uppercase">
                                            <Music size={16} /> Audio Listening Class
                                        </div>
                                        <audio className="w-full mt-1" controls src={`/storage/${activeLesson.lesson_audio_path}`} />
                                    </div>
                                )}

                                {/* 3. JIKA TIPE PDF DOKUMEN */}
                                {activeLesson.content_type === 'pdf' && activeLesson.pdf_path && (
                                    <div className="flex items-center justify-between rounded-2xl border border-gray-100 bg-white p-5 shadow-xs">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                <h4 className="text-sm font-bold text-gray-900">Attachment Material File (PDF)</h4>
                                                <p className="text-xs text-gray-400">Unduh dokumen materi pendukung modul ini</p>
                                            </div>
                                        </div>
                                        <a
                                ...
                                {/* BLOK MATERI POKOK (TEXT CONTENT) */}
                                {activeLesson.content && (
                                    <div className="space-y-4 rounded-2xl border border-gray-100 bg-white p-5 shadow-xs">
                                        ...
                                        {/* Khusus Mandarin: Jika di level lesson ada core sentence hanzi utama */}
                                        {isMandarin && activeLesson.sentence_hanzi && (
                                        ...
                                        {/* PERBAIKAN RICH-TEXT: Menggunakan dangerouslySetInnerHTML agar format HTML dari Filament RichEditor terbaca sempurna */}
                                        <div 
                                            className="prose prose-blue max-w-none text-sm whitespace-pre-line text-gray-600 raw-html-content"
                                            dangerouslySetInnerHTML={{ __html: activeLesson.content }}
                                        />
                                    </div>
                                )}

                                {/* BLOK KONDISIONAL KOSAKATA & KALIMAT (HANYA MUNCUL DI MANDARIN) */}
                                {isMandarin && (
                                    <>
                                        {/* Dropdown/List Kosa Kata Baru */}
                                        {activeLesson.vocabularies?.length > 0 && (
                                            <div className="space-y-4 rounded-2xl border border-gray-100 bg-white p-5 shadow-xs">
                                                <h3 className="flex items-center gap-2 text-xs font-bold text-gray-900 uppercase">
                                                    🔊 Vocabulary List
                                                </h3>
                                                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                                                    {activeLesson.vocabularies.map((v: any) => (
                                                        <div
                                                            key={v.id}
                                        ...
                                        {/* Contoh Penggunaan Kalimat Kompleks */}
