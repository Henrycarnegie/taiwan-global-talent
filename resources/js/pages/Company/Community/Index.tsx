...
export default function CompanyCommunityIndex({ company, initialPosts }: Props) {
    ...
    const handleToggleLike = (postId: number) => {
        router.post(`/company/community/${postId}/like`, {}, {
            preserveScroll: true,
        });
    };
    ...
    const handleCommentSubmit = (e: React.FormEvent, postId: number) => {
        ...
        router.post(`/company/community/${postId}/comment`, {
            content: commentText,
        }, {
            preserveScroll: true,
            onSuccess: () => {
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-slate-500 dark:text-slate-400">
                                Share company achievements, hiring updates, and engage with talents.
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                    <form onSubmit={handlePostSubmit} className="space-y-4">
                        <div className="flex items-start gap-3">
                            <div className="h-10 w-10 overflow-hidden rounded-xl bg-indigo-100 flex items-center justify-center font-bold text-indigo-600 dark:bg-indigo-950">
                                {company?.logo_url ? (
                            ...
                            <textarea
                                ...
                                rows={3}
                                className="w-full resize-none rounded-xl border border-slate-200 bg-slate-50 p-3 text-xs text-slate-900 focus:border-indigo-500 focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500/20 dark:border-slate-800 dark:bg-slate-950 dark:text-white"
                                required
                            />
                        </div>
                        ...
                        <div className="flex flex-wrap items-center justify-between gap-3 border-t border-slate-100 pt-3 dark:border-slate-800">
                            <div className="flex items-center gap-3">
                                ...
                                <div className="flex items-center gap-1.5 text-xs text-slate-500">
                                    ...
                                    >
                                        <option value="Hiring / Career">Hiring / Career</option>
                                ...
                                <label className="flex cursor-pointer items-center gap-1.5 rounded-lg bg-slate-100 px-3 py-1.5 text-xs font-semibold text-slate-600 hover:bg-slate-200 dark:bg-slate-800 dark:text-slate-300">
                                    ...
                                    <span>{data.media ? data.media.name : 'Attach Media'}</span>
                                    <input
                                        type="file"
                                        accept="image/*,video/*"
                                        className="hidden"
                    ...
                    {posts.map((postItem) => (
                        ...
                        >
                            ...
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-3">
                                    <div className="h-10 w-10 overflow-hidden rounded-xl bg-indigo-100 font-bold text-indigo-600 flex items-center justify-center dark:bg-indigo-950">
                                        {postItem.user.avatar_url ? (
                                            <img src={postItem.user.avatar_url} alt={postItem.user.name} className="h-full w-full object-cover" />
                                        ) : (
                                    ...
                                    <div>
                                        ...
                                        <p className="text-[11px] text-slate-500 dark:text-slate-400">
                                            {postItem.user.university} • {new Date(postItem.created_at).toLocaleDateString()}
                                        </p>
                                    </div>
                                </div>
                            ...
                            {/* Media Attachment */}
                            {postItem.media_url && (
                                <div className="overflow-hidden rounded-xl border border-slate-100 dark:border-slate-800 max-h-96">
                                    {postItem.media_type === 'video' ? (
                                        <video src={postItem.media_url} controls className="w-full object-cover" />
                                    ) : (
                                        <img src={postItem.media_url} alt="Attachment" className="w-full object-cover" />
                                    )}
                                </div>
                            )}

                            {/* Like & Comment Buttons */}
                            <div className="flex items-center gap-6 border-t border-slate-100 pt-3 dark:border-slate-800 text-xs font-semibold text-slate-500">
                                <button
                                    onClick={() => handleToggleLike(postItem.id)}
                                    className={`flex items-center gap-1.5 transition-colors ${
                                        postItem.is_liked_by_me ? 'text-rose-600' : 'hover:text-slate-900 dark:hover:text-white'
                                    }`}
                                >
                                    <Heart className={`h-4 w-4 ${postItem.is_liked_by_me ? 'fill-rose-600 text-rose-600' : ''}`} />
                                    <span>{postItem.likes_count} Likes</span>
                                </button>

                                <button
                                    onClick={() => setOpenCommentId(openCommentId === postItem.id ? null : postItem.id)}
                                    className="flex items-center gap-1.5 hover:text-slate-900 dark:hover:text-white"
                                >
                                    <MessageSquare className="h-4 w-4" />
                                    <span>{postItem.comments_count} Comments</span>
                                </button>
                            </div>
                                ...
                                <div className="space-y-3 pt-3 border-t border-slate-100 dark:border-slate-800">
                                    ...
                                    <div className="space-y-2 max-h-60 overflow-y-auto">
                                        {postItem.comments.map((comment) => (
                                            <div key={comment.id} className="rounded-xl bg-slate-50 p-3 text-xs dark:bg-slate-800/50">
                                    ...
                                    {/* Write Comment */}
                                    <form onSubmit={(e) => handleCommentSubmit(e, postItem.id)} className="flex items-center gap-2">
                                        <input
                                            type="text"
                                            value={commentText}
